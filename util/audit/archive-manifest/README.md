...
Mode: **read**. Operation: `archive-manifest`.
...
```bash
bin/archiving-utils archive-manifest --help
```
...
```text
usage: archive-manifest [-h] [--config CONFIG] [--apply] [-n] [-j JOBS] [-q]
                        [-o OUTPUT] [--output-dir OUTPUT_DIR]
                        [--against AGAINST] [--manifest MANIFEST]
                        [-S SUCCESS_LOG] [-L FAILURE_LOG] [--size SIZE]
...
options:
  ...
  --config CONFIG       JSON config, defaults to XDG_CONFIG_HOME/archiving-
                        utils/config.json
